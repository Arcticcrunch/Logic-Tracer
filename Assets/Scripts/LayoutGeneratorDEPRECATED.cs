...
public class LayoutGeneratorDEPRECATED : MonoBehaviour
{
    ...
    private void CreateGrid(int inputBusCount, bool[] truthColumn, FormulaType formulaType)
    {
    ...
    private void RenderGrids()
    {
        ...
        for (int i = 0; i < settings.GetInputsCount; i++)
        {
            ...
            rect.anchoredPosition = new Vector2(originOffset.x + i * inputBusSpacing, originOffset.y - charCellSize.y)
                + new Vector2(charCellSize.x * 0.5f, 0);
            Bus b = go.GetComponent<Bus>();
            b.lineRenerer.showNodes = false;
            b.lineRenerer.ConstructLine(Vector2.zero, new Vector2(0, -700), lineWidth);
            buses.Add(b);
        }

        GateGroup GG = new GateGroup(andGate.GetComponent<Gate>(), connectionLine.GetComponent<UILineRenerer>(), gateSize.x * 2);
        GG.ConstructGateGroup(buses, new Vector2(1000 , -250), transform, lineWidth);
